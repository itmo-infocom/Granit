...
#---------------------------------------------------------------+
#                                                               |
# Usage: create-admin-vpkshd.sh <sql commands file>             |
#                                                               |
#                                                               |
# The script is devoted to:                                     |
# creating admin user  for database  VPKSHD                     |                    |
#                                                               |
...
------------------------------------------------------------------------------------------------
  ...
  the script '`basename $0`' is designed to create admin user for database VPKSHD 
...
#
#  Create admin user for database 'vpkshd'          
#
sql_command="create-admin-vpkshd.sql"
dbname=${PGDBNAME}
username=${PGUSERNAME}
...
echo "---------------------------------------------------------"
echo " Creating admin user  for database '${VPKSHDDB}' "
echo "---------------------------------------------------------"

if ( psql -f ${sql_command} ${dbname}  ${username} ); then
  RC=$?
  echo
  echo "admin user  for database '${VPKSHDDB}' succesfully created"
else
  RC=$?
  echo
  echo "failed to create admin user  for database '${VPKSHDDB}' "
  echo "return code=${RC} "
fi
